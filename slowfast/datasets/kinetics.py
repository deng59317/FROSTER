...
def construct_loader(cfg, split, is_precise_bn=False):
    ...
    # ---- safer batch_size computation: avoid batch_size == 0 ----
    def _safe_divide_batch(total_batch):
        # Prefer NUM_SHARDS if present (per-process shards), else use NUM_GPUS
        num_shards = getattr(cfg, "NUM_SHARDS", None)
        if num_shards is None:
            num_shards = getattr(cfg, "NUM_GPUS", 1)
        try:
        ...
        try:
            ...
            total_batch = 1
        # use integer division but clamp to at least 1
        return max(1, total_batch // num_shards)

    if split == "train":
        dataset_name = cfg.TRAIN.DATASET
        batch_size = _safe_divide_batch(cfg.TRAIN.BATCH_SIZE)
        shuffle = True
        drop_last = True
    elif split == "val":
        dataset_name = cfg.TRAIN.DATASET
        ...
        batch_size = _safe_divide_batch(cfg.TEST.BATCH_SIZE)
        shuffle = False
        drop_last = False

    # Optional debug print (uncomment if you want to trace values)
    # print(f"[DEBUG] construct_loader split={split}, TRAIN.BATCH_SIZE={getattr(cfg,'TRAIN').BATCH_SIZE}, "
    #       f"TEST.BATCH_SIZE={getattr(cfg,'TEST').BATCH_SIZE if getattr(cfg,'TEST',None) else 'N/A'}, "
    #       f"NUM_GPUS={getattr(cfg,'NUM_GPUS',None)}, NUM_SHARDS={getattr(cfg,'NUM_SHARDS',None)}, "
